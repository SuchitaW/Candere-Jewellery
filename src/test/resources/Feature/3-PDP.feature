...
Feature: E-Commerce website healthcheck
  ...
  @productdescpage
  Scenario: User able to see prodct type Ring on landing page
  ...
  # to maintain this case because product cutomize change if product change
  ...
  Scenario Outline: To Verify Wishlist functionality & Login / SignUp via wishlist
    ...
    And User click on pendants on landing page
    And choose to buy the first item in list
    ...
    Examples: 
      | product | name   | Emailidmobileno           | mobileno   |
      | Chain   | QWEWZD | suchita.tayde@candere.com | 8688991212 |
